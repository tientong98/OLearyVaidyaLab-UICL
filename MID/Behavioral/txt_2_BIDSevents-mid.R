...
for(i in 1:length(subjects)){
  for(j in 1:length(runs)){
      df <- mid_allsub %>%
        filter(Subject == subjects[i], ExperimentName == runs[j]) %>%
        mutate(onset = round((Cue.OnsetTime - Fix.OnsetTime[1]) / 1000 - 12, 0)) %>%
        mutate(trial_type = ifelse(Shape == "dist/sqr.bmp" & !is.na(Tgt.RESP), "Loss0Hit", 
                            ifelse(Shape == "dist/sqr1.bmp" & !is.na(Tgt.RESP), "Loss0.2Hit",
                            ifelse(Shape == "dist/sqr2.bmp" & !is.na(Tgt.RESP), "Loss1Hit",
                            ifelse(Shape == "dist/sqr3.bmp" & !is.na(Tgt.RESP), "Loss5Hit",
                            ifelse(Shape == "dist/sqr.bmp" & is.na(Tgt.RESP), "Loss0Miss",
                            ifelse(Shape == "dist/sqr1.bmp" & is.na(Tgt.RESP), "Loss0.2Miss",
                            ifelse(Shape == "dist/sqr2.bmp" & is.na(Tgt.RESP), "Loss1Miss",
                            ifelse(Shape == "dist/sqr3.bmp" & is.na(Tgt.RESP), "Loss5Miss",
                            ifelse(Shape == "dist/cir.bmp" & !is.na(Tgt.RESP), "Gain0Hit",
                            ifelse(Shape == "dist/cir1.bmp" & !is.na(Tgt.RESP), "Gain0.2Hit",
                            ifelse(Shape == "dist/cir2.bmp" & !is.na(Tgt.RESP), "Gain1Hit",
                            ifelse(Shape == "dist/cir3.bmp" & !is.na(Tgt.RESP), "Gain5Hit",
                            ifelse(Shape == "dist/cir.bmp" & is.na(Tgt.RESP), "Gain0Miss",
                            ifelse(Shape == "dist/cir1.bmp" & is.na(Tgt.RESP), "Gain0.2Miss",
                            ifelse(Shape == "dist/cir2.bmp" & is.na(Tgt.RESP), "Gain1Miss",
                            ifelse(Shape == "dist/cir3.bmp" & is.na(Tgt.RESP), "Gain5Miss", "Fail")))))))))))))))))
      duration <- rep.int(0.25,48)
      response_time <- rep.int("n/a", 48)
      onset <- df$onset[2:49]
      trial_type <- df$trial_type[2:49]
      timing <- data.frame(onset, duration, trial_type, response_time)
      dir <- c("/Shared/oleary/functional/UICL/BIDS/")
      sub <- c("sub")
      ses <- c("ses")
      dash <- c("-")
      slash <-c("/")
      ...
      task <- c("task-mid_")
      event <- c("_events.tsv")
      write.table(timing, sep = "\t", row.names=FALSE, quote = FALSE,
                  file=paste(dir, sub, dash, subjects[i], slash, ses, dash, 
                             sessions[i], func, slash, sub, dash, subjects[i], underscore, 
                             ses, dash, sessions[i], underscore, task, runs[j], event, sep=""))
}}
